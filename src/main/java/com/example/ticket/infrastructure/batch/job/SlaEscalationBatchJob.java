...
public class SlaEscalationBatchJob {
    ...
    public void executeEscalation() {
        log.info("【バッチ開始】SLA超過チケットエスカレーション: {}", LocalDateTime.now());
        ...
        try {
            ...
            for (Ticket ticket : overdueTickets) {
                try {
                    commandService.escalateTicket(ticket.getId().getValue());
                    escalatedCount++;
                } catch (Exception e) {
                    log.error("エスカレーション失敗: チケットID={}", ticket.getId(), e);
                }
            }
            
            log.info("【バッチ完了】エスカレーション完了: {}件/{}", escalatedCount, overdueTickets.size());
        } catch (Exception e) {
